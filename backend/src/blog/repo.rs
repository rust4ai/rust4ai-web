...
pub async fn list_published(
    ...
    if let Some(tag) = tag {
        sqlx::query_as::<_, PostSummary>(
            "SELECT id, slug, title, excerpt, cover_image_url, tags, status, published_at, created_at
...
pub async fn get_published_by_slug(pool: &PgPool, slug: &str) -> Result<Option<Post>, sqlx::Error> {
    sqlx::query_as::<_, Post>(
        "SELECT id, slug, title, excerpt, body_md, cover_image_url, tags, status, published_at, created_at, updated_at
...
pub async fn insert(
    ...
    sqlx::query_as::<_, Post>(
        "INSERT INTO posts (slug, title, excerpt, body_md, cover_image_url, tags)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, slug, title, excerpt, body_md, cover_image_url, tags, status, published_at, created_at, updated_at",
    )
    .bind(slug)
    .bind(title)
    .bind(excerpt)
